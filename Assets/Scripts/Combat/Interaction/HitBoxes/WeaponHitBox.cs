...
public class WeaponHitBox : HitBox
{
    ...
    protected override void OnStrikeHurtBox(HurtBox hurtBox)
    {
        ...
        if (hurtBox.sourceObject != cnc.gameObject)
        {
            if (hurtBox.TakeDamage(this))
            {
    ...
    {
        Vector3 currentOffset = hurtBox.sourceObject.transform.position - cnc.transform.position;
       
        Vector3 direction = currentOffset.normalized;
        Vector3 facing = cnc.transform.forward;
        float currentDistance = currentOffset.magnitude-(hurtBox.CenterToBounds(cnc.transform)+characterRadius);
        Debug.Log(currentOffset.magnitude + "   " + hurtBox.CenterToBounds(cnc.transform));
